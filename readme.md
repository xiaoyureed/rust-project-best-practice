...
- [语法](#语法)
    ...
    - [基本数据类型](#基本数据类型)
        ...
        - [切片](#切片)
        - [str 字符串切片](#str-字符串切片)
        - [原生指针 raw pointer](#原生指针-raw-pointer)
    ...
    - [字符串](#字符串)
        ...
        - [字符串转换](#字符串转换)
        - [格式化](#格式化)
        - [调试打印复合类型 Debug Display](#调试打印复合类型-debug-display)
        - [问号操作符 多次写](#问号操作符-多次写)
    - [集合](#集合)
        - [Vec 动态数组](#vec-动态数组)
...
- [和 js JavaScript nodejs 交互](#和-js-javascript-nodejs-交互)
- [和 java 交互](#和-java-交互)
...
比如: String 和 Vec 类型 也是一种智能指针, 它们也都实现了 Deref和Drop
...
通过 Box，开发者可以方便无痛地使用堆内存， 并且无须手工释放堆内存， 可以确保内存安全
...
```rs
    ...
    #[derive(Debug, Clone)] // 加上 clone trait
    struct Person {
        name: String,
        age: i32,
    }
    ...
    fn it_works() {
        ...
        p1 = p2.clone(); // 显式调用 clone(), 若希望rust 碰到 等号了就自动隐式调用 clone() , 需要给结构体加上 derive(Clone, Copy), 注意copy trait 无法单独使用, 必须在 Clone trait 存在的情况下才能使用
        println!("p1 = {:?}", p1);
        println!("p1 = {:?}", p2);
    }
